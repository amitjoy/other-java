...
public class DeviceGroupsViewPart {
	...
	public void postConstruct(Composite parent) {
		...
		viewer.getTable().addSelectionListener(
				new DevicesGroupsViewerSelectionListener(viewer,
						devicesGroupsView, devicesView, sensorsView,
						partService, selectionService, context));
		...
		deviceGroupsNameColumnViewer.getColumn().setText("Device Group Name");
	...
	private void hideOtherParts() {
		partService.hidePart(devicesView);
		partService.hidePart(sensorsView);
	...
	@Inject
	@Optional
	public void listDevicesGroups(
			@UIEventTopic(EventConstants.SHOW_DEVICES_GROUPS) Object object,
		...
		dialog.run(false, false, new IRunnableWithProgress() {
			...
			public void run(IProgressMonitor monitor)
				...
				monitor.beginTask("Fetching Device Groups Names", 100);
				for (int i = 0; i < 5; i++) {
